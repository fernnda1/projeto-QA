class Category
  ...
  action :edit, only: %w(show), on: :member, position: :header, layout: :default, click: {redirect_to: "edit"}, submit: {put: "..", data: {:category => ":resource"}, success: {redirect_to: "..", toast: :success}, error: {toast: :error, update: "components"}}
  action :edit_cancel, only: %w(edit), on: :member, position: :footer, click: {redirect_to: ".."}
  action :update, only: %w(edit), on: :member,position: :footer, click: {submit: "edit_category"}
  action :new, only: %w(index), on: :collection, position: :header, layout: :default, click: {redirect_to: "new"}, submit: {post: "..", data: {:category =>  ":resource"}, success: {redirect_to: "..", toast: :success}, error: {toast: :error, update: "components"}}
  action :new_cancel, only: %w(new), on: :member,position: :footer, click: {redirect_to: ".."}
  action :create, only: %w(new), on: :member,position: :footer, click: {submit: "new_category"}
  ...
  # action :edit, only: %w(show), on: :member, position: :footer, layout: :modal, click: {redirect_to: "edit", open_in: :dialog}, submit: {put: "..", data: {:category => ":resource"}, success: {redirect_to: "..", open_in: :dialog, toast: :success}, error: {toast: :error, update: "components"}}
  # action :edit_cancel, only: %w(edit), on: :member, position: :footer, click: {redirect_to: "..", open_in: :dialog}
  # action :update, only: %w(edit), on: :member,position: :footer, click: {submit: "edit_category"}
  # action :new, only: %w(index), on: :collection, position: :header, layout: :modal, click: {redirect_to: "new", open_in: :dialog}, submit: {post: "..", data: {:category =>  ":resource"}, success: {close: :dialog, toast: :success}, error: {toast: :error, update: "components"}}
  ...
  #   - render_if: condição de renderização no frontend
  # -----------------------------------------------------------------------------------------------------
  filters :category_filters, submit_on_input: true do
  ...
  #     }
  #}
  list :category_table, type: :table, sortable: :sort, thead: true, paginate: true, paginate_per: 10 do
  #   add :photo_url
  #   add :id, sort: true
  ...
  # list :category_timeline, type: :timeline, header: {html: "this.resource.date", size: 1}, title: "this.resource.id", icon: {name: 'this.resource.icon_info', background: 'this.resource.icon_color', color: 'white'}, line: {color: 'light', size: 2}, styles: {background: 'white', border: {top: 1, right: 1, left: 1, bottom: 1}, border_color: :light, rounded: {top_left: 1, top_right: 1, bottom_left: 1, bottom_right: 1}}, click_item: {redirect_to: ":resource.id"}, paginate: true, paginate_per: 10 do
  ...
  # end
  # list :category_kanban, type: :kanban, sortable: :sort, group: @@group, draggable: true, drop_item: @@drop_item, column_height: "calc(100vh -360px)" do
  #   add :photo_url, component: :image, position: :left, image_size: 2, styles: {padding: {left: 2, right: 2, top: 2, bottom: 2}, rounded: {top_left: 2, top_right: 2, bottom_left: 2, bottom_right: 2}}
  #   add :id, component: :text, type: :small, styles:{color: :primary}, size: 12,styles: {color: :primary, padding: {left: 2, right: 2, top: 2, bottom: 0}}
  #   add :textarea, component: :text, type: :small, size: 12,styles: {padding: {left: 2, right: 2, top: 0, bottom: 0}}
  #   add :date, component: :text, type: :small,styles: {padding: {left: 2, right: 2, top: 0, bottom: 2}}
  # end
  # list :category_list, type: :cards, sortable: :sort, group: @@group, draggable: true, drop_item: @@drop_item, column_height: "calc(100vh -360px)" do
  #   add :photo_url, component: :image, position: :left, image_size: 2, styles: {padding: {left: 2, right: 2, top: 2, bottom: 2}, rounded: {top_left: 2, top_right: 2, bottom_left: 2, bottom_right: 2}}
  #   add :id, component: :text, type: :small, styles:{color: :primary}, size: 12,styles: {color: :primary, padding: {left: 2, right: 2, top: 2, bottom: 0}}
  #   add :textarea, component: :text, type: :small, size: 12,styles: {padding: {left: 2, right: 2, top: 0, bottom: 0}}
  #   add :date, component: :text, type: :small,styles: {padding: {left: 2, right: 2, top: 0, bottom: 2}}
  # end
  # list :category_calendar, size: 12, type: :calendar, calendar_data: {title: :name, start: :date_time, end: :date_time}, calendar_options: {initialView: 'month', headerToolbar: {
  #     left: 'prev,next today',
  #     center: 'title',
  #     right: 'month,day,week' 
  # }}, styles: {margin: {top: 2, right: 2}}, click_item: {redirect_to: ":resource.id", open_in: :dialog} do
  # end
  # list :category_cards, type: :cards, size: 12, card_size: 3, paginate: true, paginate_per: 20, click_item: {redirect_to: ":resource.id", open_in: :dialog}, styles: {vertical_align: {body: 'center'}} do
